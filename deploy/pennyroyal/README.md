...
## Small prompt-rule screening

`benchmark-prompt-rules.py` compares a minimal coding system prompt with the same prompt plus five short execution rules. It does not reproduce a vendor's full prompt, load Pi's prompt, enable extra agents, or update production configuration. The 16 fixed tasks include 12 quoting/compatibility cases and four ordinary functional controls, with two seeds per arm (64 sequential requests). Each arm receives identical task text, temperature 1, medium reasoning and an 8192-token output limit. Arm order alternates, truncation counts as failure, and no test feedback or repair attempts are provided.

Before measuring, run `docker pull python:3.9-slim-bookworm` and `python3 deploy/pennyroyal/benchmark-prompt-rules.py --self-test`. Every reference solution must pass and every deliberate wrong-output solution must fail. Candidate code runs inside a local, non-root, network-disabled, read-only Docker container with memory/CPU/process limits and an ephemeral tmpfs. Only task payloads enter the container; no host paths or credentials are mounted. The Docker daemon must be on the local build machine. The resolved image ID is pinned for each run.

Run with `--url API_BASE --output OUTSIDE_REPOSITORY/report.json`. The report retains the frozen prompt/test manifest, its hash, responses, usage, timing, syntax results and functional oracles. Cache is not flushed on the shared production model, so timing includes cache and concurrent-traffic effects. The two samples per task are not independent tasks; task-level comparisons are reported separately. There is no equal-length neutral-prompt arm. This is targeted screening, not a general capability benchmark or proof of equivalence. Adoption requires replicated gains on fresh holdouts with no observed regressions; the script never installs rules automatically.
