...
底栏常驻显示 `tok/s`，没有生成计时时显示 `— tok/s`。Pi 流式输出时以 `~` 标记估算速度，消息完成后用服务商返回的输出 token 数校准并保留结果。计算为输出 token 数除以从首个输出到消息完成的秒数，排除首 token 等待和工具执行时间；这是客户端观察的生成吞吐，不等同服务商内部推理计时。不同会话独立统计，历史会话没有计时记录时不编造速度。可在界面布局设置中调整该独立底栏项。

在「设置 → 界面布局」中可调整整页缩放：90%、100%、110%、125%、150%、175%，默认 100%。这是服务端全局设置，所有在线浏览器立即同步，新打开的页面和服务重启后也沿用该值；保存于 `<dataDir>/ui-settings.json`（默认 `~/.pi-web/ui-settings.json`），不使用浏览器本地偏好或新增快捷键。

消息默认贴住最新底部，无平滑滚动动画。主动向上查看历史或使用搜索跳转才暂停跟随并显示「回到底部」；切换会话、返回聊天视图或重新显示浏览器页面后恢复最新消息。
