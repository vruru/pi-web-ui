...
- `submit_task`: supply `task`, optional source text in `context`, optional `image_paths` (up to four absolute PNG/JPEG/WebP files, each at most 10 MiB), and optional per-call `max_tokens` (64–16384, default 8192). Immediately returns a task ID. Only explicitly attached files are read and uploaded. The model has no independent filesystem or execution tools. Optional `role` selects `generate` (default), `review`, or `revise`; the latter two require `parent_task_id` and inherit original context/images.
...
MCP calls explicitly default to `reasoning: "off"`, sending `chat_template_kwargs.enable_thinking: false`. This keeps backend default thinking from consuming the entire output budget before producing a deliverable. Set `reasoning: "on"` explicitly for reasoning-heavy work and budget accordingly; each stage has its own setting. This does not change direct Pi/model clients or the inference server's defaults. Disabling thinking is a reliability tradeoff, not an accuracy improvement claim.
