...
- macOS 网页服务可通过专用 Herdr 会话启动，保留真实父窗格身份和 launchd 保活，让 pi-herdr-agents 能创建子代理并自动回传结果。
...
- 模板卡片按聊天面板实际宽度自动换列，修复界面放大、侧栏占位及窄窗口下网格超出面板、两侧内容被裁切的问题。

- 精简核心版本弹框：保留同行标题与关闭按钮，仅显示版本、检查时间和操作；可用按钮采用淡绿色凸起玻璃样式，禁用按钮为红色平面玻璃样式。
...
- 底栏 tok/s 使用与其他普通统计项一致的文字颜色，避免主题强调色影响可读性。
- 插件市场清单同步不再重载运行中的插件，保留插件状态与工作区监听；仅安装或更新插件后才重载。插件冒烟测试隔离外部目录，并覆盖启动同步、手动同步与目录切换的完整链路。
