...
Built-in themes ship in the npm package (`themes/`, 25 palettes). The picker shows the named palettes (`nord`, `tokyo-night`, `catppuccin`, `one-dark`, `solarized-light`, `geist`, `ayu-light`, `claude-code-dark`, `vs-code-dark`, …) under a **Classics** heading, then the built-in presets under **Original**: `white` / `mist` / `paper` / `sakura` (light), `cyberpunk` / `dazzle` / `dark-teal` (dark), `md-preview`, and the wallpaper-friendly `translucent` / `transparent` (pair with a chat wallpaper). The theme picker lives in the top bar (🌞 icon); the current choice is stored per browser in `localStorage`.
