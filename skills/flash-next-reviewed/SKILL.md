...
1. Read relevant repository material. Set the task, constraints, permitted file scope, and acceptance checks. Submit with `role: "generate"`, `task`, and `context`. Include images only if needed. Choose a sufficient `max_tokens` output allowance. MCP defaults to `reasoning: "off"` to reserve the budget for final text; use `reasoning: "on"` only deliberately for harder work. Each stage has its own setting, reported in its snapshot.
